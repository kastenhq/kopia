...
func (p *Profile) IsValidPassword(password string) bool {
	...
	if invalidProfile {
		...
		// if the user profile is invalid (either empty profile or invalid password hash version),
		// return false but use the same amount of time as when we compare against valid user
		// to avoid revealing whether the user account exists.
