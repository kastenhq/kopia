...
func (s *contentManagerSuite) TestAutoCompressionOfMetadata(t *testing.T) {
	...
	st := blobtesting.NewMapStorage(data, nil, nil)
	bm := s.newTestContentManager(t, st)
	verifyMetadataCompression(ctx, t, bm, compression.HeaderZstdFastest)
}

func (s *contentManagerSuite) TestCompressionSettingOfMetadata(t *testing.T) {
	ctx := testlogging.Context(t)
	data := blobtesting.DataMap{}
	st := blobtesting.NewMapStorage(data, nil, nil)
	// Set compression algorithm
	bm := s.newTestContentManagerWithTweaks(t, st, &contentManagerTestTweaks{
		metadataCompression: "zstd-best-compression",
	})
	verifyMetadataCompression(ctx, t, bm, compression.HeaderZstdBestCompression)

	// Disable metadata compression
	bm2 := s.newTestContentManagerWithTweaks(t, st, &contentManagerTestTweaks{
		metadataCompression: "none",
	})
	verifyMetadataCompression(ctx, t, bm2, NoCompression)
}
...
func (s *contentManagerSuite) newTestContentManagerWithTweaks(t *testing.T, st blob.Storage, tweaks *contentManagerTestTweaks) *WriteManager {
	...
	if tweaks.metadataCompression != "" {
		mp.MetadataCompression = tweaks.metadataCompression
	}
...
func verifyMetadataCompression(ctx context.Context, t *testing.T, bm *WriteManager, comp compression.HeaderID) {
	//nolint:lll
